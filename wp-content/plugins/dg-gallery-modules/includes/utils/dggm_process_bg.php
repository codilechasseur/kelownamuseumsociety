...
class DGGM_BG_PROCESS {
    ...
    function set_style() {
        ...
        $background_size = $this->dg_process_values($this->settings['_background_image_size']);
        $background_position = $this->dg_process_values($this->settings['_background_image_position']);

        if($this->dg_process_values($this->settings['_background_image_position']) === 'custom') {
            $background_position = sprintf('%1$s %2$s', $this->settings['_position_horizontal'], $this->settings['_position_vertical']);
        }
        if($this->dg_process_values($this->settings['_background_image_size']) === 'custom') {
        ...
        if ($this->settings['_use_gradient'] === 'on' || $this->settings['_background_image'] !== '') {
            ET_Builder_Element::set_style($this->options['render_slug'], array(
                ...
                'declaration' => sprintf( 'background-image: %1$s %2$s;
                    ...
                    $background_size,
                    $background_position,
                    $this->dg_process_values($this->settings['_background_image_repeat'])
                )
            ));
        }
    ...
    private function set_hover_styles() {
        $background_size = $this->dg_process_values($this->settings_hover['_background_image_size']);
        $background_position = $this->dg_process_values($this->settings_hover['_background_image_position']);

        if($this->dg_process_values($this->settings_hover['_background_image_position']) === 'custom') {
            $background_position = sprintf('%1$s %2$s', $this->settings_hover['_position_horizontal'], $this->settings_hover['_position_vertical']);
        }
        if($this->dg_process_values($this->settings_hover['_background_image_size']) === 'custom') {
        ...
        if ($this->settings_hover['_use_gradient'] === 'on' || $this->settings_hover['_background_image'] !== '') {
            ET_Builder_Element::set_style($this->options['render_slug'], array(
                ...
                'declaration' => sprintf( 'background-image: %1$s %2$s;
                    ...
                    $background_size,
                    $background_position,
                    $this->dg_process_values($this->settings_hover['_background_image_repeat'])
                ),
            ));
        }
    }
    ...
    private function background($type = 'default') {
        if ( $this->settings['_above_image'] === 'on' ) {
            return sprintf('%2$s%3$s %1$s',
                $this->dg_background_image($type),
                $this->dg_background_gradient($type),
                $this->seperator
            );
        } else {
     ...
     * Background image
     */
    private function dg_background_image($type) {
     ...
     * 
     */
    private function dg_background_gradient($type) {
        $settings = $type === 'default' ? $this->settings : $this->settings_hover;
        ...
        if ($settings['_use_gradient'] === 'on' ) {
            if ( $settings['_gradient_type'] !== 'radial') {
                ...
                return sprintf('radial-gradient( circle at %3$s, %1$s %4$s, %2$s %5$s )', 
                    $settings['_color_gradient_1'],
                    $settings['_color_gradient_2'],
                    $this->dg_process_values($settings['_radial_direction']),
                    $settings['_start_position'],
                    $settings['_end_position']
                );
            }
     ...
     * Process transition
     */
    public function dg_process_transiton($options) {
