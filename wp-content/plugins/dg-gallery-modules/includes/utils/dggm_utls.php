...
require_once ( DGGM_MAIN_DIR . '/includes/utils/dggm_background.php');
require_once ( DGGM_MAIN_DIR . '/includes/utils/dggm_process_bg.php');
require_once ( DGGM_MAIN_DIR . '/includes/utils/dggm_button.php');
...
trait DGGM_UTLS {
     ...
     */
    function dg_add_btn_content($options = []) {

        if (empty($this->button)) {
            $this->button = new DGCM_BUTTON($this);
        }
     ...
     * @return Array of settings
     */
    function dg_add_btn_styles($options = []) {
     ...
     * @return Void
     */
    function dg_process_btn_styles($options = []) {
    ...
    function dg_margin_padding(&$fields, $options, $type ) {
        ...
        $fields[$key] = array(
            'label'				=> sprintf(esc_html__('%1$s %2$s', 'dggm-gallery-modules'), $options['title'], ucwords($type)),
            'type'				=> 'custom_margin',
     ...
     * @return Array $fields
     */
    function dg_transition_options($options = []) {
        $fields = array ();
        $default = array (
            'key'               => '',
            'toggle_slug'       => '',
        ...
        $fields[$key . '_transition_duration'] = array (
            'label'             => esc_html__( 'Transition Duration', 'dggm-gallery-modules' ),
				'type'              => 'range',
        ...
        $fields[$key . '_transition_delay'] = array (
            'label'             => esc_html__( 'Transition Delay', 'dggm-gallery-modules' ),
				'type'              => 'range',
                'toggle_slug'       => $toggle_slug,
                'tab_slug'          => $tab_slug,
				'default'           => '0ms',
        ...
        $fields[$key . '_transition_curve'] = array (
            'default'         => 'ease_in_out',
            'label'           => esc_html__( 'Transition Speed Curve', 'dggm-gallery-modules' ),
            'type'            => 'select',
            'options'         => array(
                'ease'          => esc_html__( 'Ease', 'dggm-gallery-modules' ),
                'ease_in'       => esc_html__( 'Ease In', 'dggm-gallery-modules' ),
                'ease_in_out'   => esc_html__( 'Ease In Out', 'dggm-gallery-modules' ),
                'ease_out'      => esc_html__( 'Ease Out', 'dggm-gallery-modules' ),
                'linear'        => esc_html__( 'Linear', 'dggm-gallery-modules' ),
                'bounce'        => esc_html__( 'Bounce', 'dggm-gallery-modules')
            ),
            'toggle_slug'     => $toggle_slug,
            'tab_slug'        => $tab_slug,
            'show_if'             => $show_if,
     ...
     * @return Void
     */
    function dg_process_transition($options = []) {
        $default = array(
            'render_slug'       => '',
            'slug'              => '',
            'selector'          => '',
        ...
        $transition_fn = $this->dg_transition($this->props[$slug . '_transition_curve']);
     ...
     */
    function dg_add_icon_settings($options = array()) {
        $fields = array ();
        $default = array (
            'title'             => '',
            'image_title'       => '',
        ...
        if ($image === true) {
            $fields[$prefix.'image'] = array (
                'label'                 => esc_html__( $image_title, 'dggm-gallery-modules' ),
				'type'                  => 'upload',
				'upload_button_text'    => esc_attr__( 'Upload an image', 'dggm-gallery-modules' ),
				'choose_text'           => esc_attr__( 'Choose an Image', 'dggm-gallery-modules' ),
				'update_text'           => esc_attr__( 'Set As Image', 'dggm-gallery-modules' ),
                'toggle_slug'           => $toggle_slug,
                'tab_slug'              => $tab_slug,
            ...
            if($image_alt === true) {
                $fields[$prefix . 'alt_text'] = array (
                    'label'                 => esc_html__( 'Image Alt Text', 'dggm-gallery-modules' ),
                    'type'                  => 'text',
                    'toggle_slug'           => 'image',
                    'depends_show_if_not'   => 'on'
                );
        ...
        if ($on_off === true) {
            $fields[$prefix.'use_icon'] = array(
				'label'                 => esc_html__( $button_text, 'dggm-gallery-modules' ),
				'type'                  => 'yes_no_button',
        ...
        if ($icon_alignment === true) {
            $fields[$prefix.'icon_align'] = array (
                'label'             => esc_html__( 'Icon Align', 'dggm-gallery-modules' ),
				'type'              => 'text_align',
        ...
        if ($image_styles === true) {
            $fields[$prefix.'image_align'] = array (
                'label'             => esc_html__( 'Image Alignment', 'dggm-gallery-modules' ),
				'type'              => 'text_align',
				'options'           => et_builder_get_text_orientation_options(array('justified')),
				'tab_slug'          => $img_tab,
            ...
            $fields[$prefix.'full_width']    = array (
                'label'             => esc_html__('Force Full Width', 'dggm-gallery-modules'),
                'type'              => 'yes_no_button',
                'options'           => array(
					'off' => esc_html__( 'No', 'dggm-gallery-modules' ),
					'on'  => esc_html__( 'Yes', 'dggm-gallery-modules' ),
            ...
            if ($max_width === true) {
                $fields[$prefix.'max_width'] = array(
                    'label'             => esc_html__( 'Max Width', 'dggm-gallery-modules' ),
                    'type'              => 'range',
                    'toggle_slug'       => $img_toggle,
                    'tab_slug'          => $img_tab,
                    'default'           => '100%',
        ...
        if ($icon_bg === true) {
            $fields[$prefix.'icon_bg'] = array (
				...
				'label'             => esc_html__( 'Icon Background', 'dggm-gallery-modules' ),
				'type'              => 'color-alpha',
				'depends_show_if'   => 'on',
				'tab_slug'          => $tab_slug,
                'toggle_slug'       => $toggle_slug,
        ...
        if ($circle_icon === true) {
            $fields[$prefix.'circle_icon'] = array(
				'label'                 => esc_html__( 'Circle Icon', 'dggm-gallery-modules' ),
				'type'                  => 'yes_no_button',
				'option_category'       => 'basic_option',
				'options'               => array(
					'off' => esc_html__( 'No', 'dggm-gallery-modules' ),
    ...
    function dg_text_clip($options = array()) {
        ...
        $fields[$key . '_enable_clip'] = array(
            'label'             => esc_html__( 'Enable Clip', 'dggm-gallery-modules' ),
            'type'              => 'yes_no_button',
            'options'           => array(
                'off' => esc_html__( 'No', 'dggm-gallery-modules' ),
                'on'  => esc_html__( 'Yes', 'dggm-gallery-modules' ),
        ...
        $fields[$key . '_enable_bg_clip'] = array(
            'label'             => esc_html__( 'Enable Background Clip', 'dggm-gallery-modules' ),
            'type'              => 'yes_no_button',
            'options'           => array(
                'off' => esc_html__( 'No', 'dggm-gallery-modules' ),
                'on'  => esc_html__( 'Yes', 'dggm-gallery-modules' ),
        ...
        $fields[$key . '_fill_color'] = array(
            'label'             => esc_html__( 'Fill Color', 'dggm-gallery-modules' ),
            'type'              => 'color-alpha',
            'toggle_slug'       => $toggle_slug,
            'tab_slug'          => $tab_slug,
            'hover'             => 'tabs',
        ...
        $fields[$key . '_stroke_color'] = array(
            'label'             => esc_html__( 'Stroke Color', 'dggm-gallery-modules' ),
            'type'              => 'color-alpha',
            'toggle_slug'       => $toggle_slug,
            'tab_slug'          => $tab_slug,
            'hover'             => 'tabs',
        ...
        $fields[$key . '_stroke_width'] = array(
            'label'             => esc_html__( 'Stroke Width', 'dggm-gallery-modules' ),
            'type'              => 'range',
            'toggle_slug'       => $toggle_slug,
            'tab_slug'          => $tab_slug,
            'default'           => '1px',
    ...
    function dg_process_text_clip($options = array()) {
        ...
        if ($this->props[$slug . '_enable_clip'] === 'on') {
            ...
            $this->dg_apply_single_value(array(
                'render_slug'       => $render_slug,
     ...
     * Process string attr
     */
    function dg_process_string_attr($options = array()) {
        $default = array(
            'render_slug'       => '',
            'slug'              => '',
            'type'              => '',
        ...
        $desktop  =  !empty($this->props[$slug]) ? 
            $this->dg_process_values($this->props[$slug]) : $default;
        $tablet   =  !empty($this->props[$slug.'_tablet']) ? 
            $this->dg_process_values($this->props[$slug.'_tablet']) : $desktop;
        $phone   =  !empty($this->props[$slug.'_phone']) ? 
            $this->dg_process_values($this->props[$slug.'_phone']) : $tablet;
    ...
    function dg_process_maxwidth($options = array()) {
        ...
        $phone = $this->dg_check_values($tablet, $this->props[$max_width.'_phone']);
        ...
        if (et_builder_is_hover_enabled( $max_width, $this->props ) && isset($this->props[$max_width.'__hover']) && $hover !== '') {
            $hover_value = $this->props[$max_width.'__hover'];
            if ( !empty($hover_value)) {
                ET_Builder_Element::set_style($render_slug, array(
                    'selector' => $hover,
                    'declaration' => sprintf('%1$s:%2$s %3$s;', 'max-width', $hover_value, $important_text),
                ));
            } 
        }
        if ($alignment === true) {
            ...
            $phone_align = $this->dg_check_values($tablet_align, $this->props[$align.'_phone']);
            ET_Builder_Element::set_style($render_slug, array(
                'selector' => $selector,
     ...
     * old version
     */
    function dg_apply_single_value($options = array()) {
     ...
     * 
     */
    function dg_process_range ( $options = array() ) {
        $default = array(
            'render_slug'       => '',
            'slug'              => '',
            'type'              => '',
     ...
     * 
     */
    function dg_process_oposite_value( $value, $ops = false) {
        if ($ops === true) {
            if (intval($value) > 0) {
                $value = '-' . $value;
            } else {
     ...
     */
    function dg_process_transform( $options = array () ) {
        ...
        foreach ( $transforms as $t ) {
            ...
            $desktop = isset($this->props[$slug]) && $this->props[$slug] !== '' ?
                $this->dg_process_oposite_value($this->props[$slug], $oposite) : $default;
            $tablet = isset($this->props[$slug . '_tablet']) && $this->props[$slug . '_tablet'] !== '' ?
                $this->dg_process_oposite_value($this->props[$slug . '_tablet'], $oposite) : $desktop;
            $phone = isset($this->props[$slug . '_phone']) && $this->props[$slug . '_phone'] !== '' ?
                $this->dg_process_oposite_value($this->props[$slug . '_phone'], $oposite) : $tablet;
            $hover = isset($this->props[$slug . '__hover']) && $this->props[$slug . '__hover'] !== '' ?
                $this->dg_process_oposite_value($this->props[$slug . '__hover'], $oposite) : $desktop;
     ...
     * process css filter
     */
    function dg_process_filter ( $options = array() ) {
     ...
     * @return Flex
     */
    function dg_process_flex_values($value) {
        $array = array(
            'flex_left'     =>  'row',
            'flex_top'      =>  'column',
            'flex_right'    =>  'row-reverse',
     ...
     * @return void
     */
    function dg_process_bg( $options = array() ) {
        $default = array(
            'render_slug'       => '',
            'slug'              => '',
            'selector'          => '',
        ...
        $options        = wp_parse_args( $options, $default );
        $background = new DGGM_BG_PROCESS($this, $options);
     ...
     * @return void
     */
    function dg_background_transition ($options) {
        $background = new DGGM_BG_PROCESS($this, $options);
        return $background->dg_process_transiton($options);
    }
     ...
     * 
     */
    function dg_fix_border_transition($fields, $slug, $selector) {
        // all
     ...
     * 
     */
    function dg_fix_box_shadow_transition($fields, $slug, $selector) {
        // all
     ...
     */
    function dg_fix_carousel_item_initial_width($options = array()) {
    ...
    function df_background_settings($options = array()) {
        ...
        $background["{$option_name}_color"] = array(
            'label'             => esc_html__( $title, 'dggm-gallery-modules' ),
            'description'       => esc_html__( 'Adjust the background style of the Badge 1 by customizing the background color, gradient, and image.', 'dggm-gallery-modules' ),
