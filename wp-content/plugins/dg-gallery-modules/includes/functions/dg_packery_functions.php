 ...
 * @return $options
 */
function dg_pg_options($object, $images = null) {
    $options = array(
 ...
 */
function dg_pg_render_gallery_markup($options=[], $load_more_type = false, $load_count = 0){
    $default = array(
        'gallery' => '',
        'pg_layout' => '',
        'margin' => '',
	...
	$image_ids = explode( ',', ! empty( $gallery ) ? $gallery : '' );
	$images = '';
    $_i = 0;
    ...
    foreach( $image_ids as $id ) {
        ...
        $image = sprintf('<div class="dg_pg_item%11$s" data-src="%8$s" %4$s %10$s>
                <figure class="c4-izmir dg_pg_image %6$s %7$s %9$s" %10$s style="background-image:url(%1$s);">
                    %12$s
                    <figcaption class="dg_pg_content %5$s">
                        %3$s
                    </figcaption>
                </figure></div>', 
            esc_attr( $image_url ), 
            ...
            $custom_url,
            dg_pg_layout_classes($layout_index, $pg_layout),
            $overlay === 'on' ? '<span class="dg-overlay"></span>' : '',
            $use_lightbox
        );
 ...
 */
add_action('wp_ajax_dg_pg_render_image', 'dg_pg_render_image_gallery_fb_callback');
function dg_pg_render_image_gallery_fb_callback() {
    ...
    $gallery_array = dg_pg_options($data);
    $images = dg_pg_render_gallery_markup($gallery_array);
    
    wp_send_json_success($images);
}

function dg_pg_galler_load_actions( $actions ) {
	$actions[] = 'dg_pg_render_image';

	return $actions;
}
add_filter( 'et_builder_load_actions', 'dg_pg_galler_load_actions' );
 ...
 * @return json response
 */
add_action('wp_ajax_dg_pg_fetch', 'dg_pg_load_more_image');
add_action('wp_ajax_nopriv_dg_pg_fetch', 'dg_pg_load_more_image');
function dg_pg_load_more_image() {
    ...
    $gallery_array = dg_pg_options($options, $_images);

    $images = dg_pg_render_gallery_markup($gallery_array, true, $loaded);
 ...
 */
function dg_pg_layout_classes($index, $pg_layout) {
    $classes = '';

    if ($pg_layout === 'one') {
        if ($index%8 === 3 || $index%8 === 5) {
            $classes = ' dg_pg_item--width-height2';
        } else if ($index%8 === 4 || $index%8 === 0) {
            $classes = ' dg_pg_item--width2';
        }
        ...
        if (dg_get_lastDigit($index) === 3 || dg_get_lastDigit($index) === 6) {
            $classes = ' dg_pg_item--width-height2';
        } 
