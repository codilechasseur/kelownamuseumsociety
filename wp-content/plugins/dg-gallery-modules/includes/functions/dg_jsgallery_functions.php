 ...
 * @return $options
 */
function dg_jsg_options($object, $images = null) {
    $options = array(
 ...
 */
function dg_jsg_render_gallery_markup($options=[], $load_more_type = false){
    $default = array(
        'gallery' => '',
        'margin' => '',
        'show_caption' => 'off',
	...
	if ( empty( $gallery ) ) {
		$images .= sprintf( '<figure class="c4-izmir dg_jsg_image">  
                    <img class="jsg-image" src="%1$s"/>
                </figure>',
			$default_image
		);
    ...
    foreach( $image_ids as $id ) {
        ...
        if(('on' == $show_content_lg || 'on' == $show_caption) && $details) {
            $caption = $details->post_excerpt !== ''?
                sprintf('<div class="%3$s %4$s"><%2$s class="dg_jsg_caption">%1$s</%2$s></div>', 
        ...
        if(('on' == $show_content_lg || 'on' == $show_description) && $details) {
            $description = $details->post_content !== ''?
                sprintf('<div class="%3$s %4$s"><%2$s class="dg_jsg_description">%1$s</%2$s></div>', 
	    ...
	    if ( empty( $image_url_array ) || ! is_array( $image_url_array ) ) {
		    $image_url_array    = [];
		    $image_url_array[1] = '';
		    $image_url_array[2] = '';
	    }
        ...
        $image = sprintf('<figure class="c4-izmir dg_jsg_image %6$s %7$s %9$s %11$s" data-src="%8$s" %4$s %10$s>
                    %12$s
                    
                    <img class="jsg-image" src="%1$s" alt="%2$s" class="" width="%13$s" height="%14$s"/>
                    <figcaption class="dg_jsg_content %5$s">
                        %3$s
                    </figcaption>
                </figure>', 
            esc_attr( $image_url ), 
 ...
 */
add_action('wp_ajax_dg_jsg_render_image', 'dg_jsg_render_image_gallery_fb_callback');
function dg_jsg_render_image_gallery_fb_callback() {
    ...
    $gallery_array = dg_jsg_options($data);
    $images = dg_jsg_render_gallery_markup($gallery_array);
    
    wp_send_json_success($images);
}

function dg_jsg_galler_load_actions( $actions ) {
	$actions[] = 'dg_jsg_render_image';

	return $actions;
}
add_filter( 'et_builder_load_actions', 'dg_jsg_galler_load_actions' );
 ...
 * @return json response
 */
add_action('wp_ajax_dg_jsg_fetch', 'dg_jsg_load_more_image');
add_action('wp_ajax_nopriv_dg_jsg_fetch', 'dg_jsg_load_more_image');
function dg_jsg_load_more_image() {
    ...
    $gallery_array = dg_jsg_options($options, $_images);

    $images = dg_jsg_render_gallery_markup($gallery_array, true);
