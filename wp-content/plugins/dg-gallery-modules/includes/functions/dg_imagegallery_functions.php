 ...
 * @return $options
 */
function dg_ig_options($iamges, $object) {
    $options = array(
 ...
 */
add_action('wp_ajax_dg_image_gallery', 'dg_image_gallery');
function dg_image_gallery() {
    ...
    $gallery = dg_ig_render_images( $options );

    wp_send_json_success($gallery);
}

function dg_display_galler_load_actions( $actions ) {
	$actions[] = 'dg_image_gallery';

	return $actions;
}
add_filter( 'et_builder_load_actions', 'dg_display_galler_load_actions' );
 ...
 */
add_action('wp_ajax_dg_image_gallery_fetch', 'dg_image_gallery_more_image');
add_action('wp_ajax_nopriv_dg_image_gallery_fetch', 'dg_image_gallery_more_image');
function dg_image_gallery_more_image() {
    ...
    $images = dg_ig_render_images( $options, true );

    wp_send_json_success($images);
}

add_action('wp_ajax_dg_image_gallery_category_data_fetch', 'dg_image_gallery_category_data_fetch');
add_action('wp_ajax_nopriv_dg_image_gallery_category_data_fetch', 'dg_image_gallery_category_data_fetch');
function dg_image_gallery_category_data_fetch() {
	if (isset($_POST['et_frontend_nonce']) && !wp_verify_nonce( sanitize_text_field($_POST['et_frontend_nonce']), 'et_frontend_nonce' )) {
		wp_die();
	}
	...
	foreach ($tax_array as $tax_id) {
		...
		if ($media_items) {
			$gallery_title = get_term($tax_id, DiviGear_Media_Category::TAXONOMY)->name;
 ...
 */
function dg_ig_render_images( $options=[], $load_more_type = false ) {
    ...
    foreach( $images_array as $id => $value ) {
        ...
        if(('on' === $use_lightbox_content || 'on' === $show_caption) && $details){
            $caption = $details->post_excerpt !== ''?
                sprintf('<div class="%3$s %4$s"><%2$s class="dg_ig_caption">%1$s</%2$s></div>',
                    wp_kses_post($details->post_excerpt, 'dggm-gallery-modules'),
                    esc_attr($caption_tag),
                    esc_attr($always_title),
                    esc_attr( 'on' !== $show_caption ? 'dg-hide-title' : '')
                    ) : '';
        }
        if(('on' === $use_lightbox_content || 'on' === $show_description) && $details) {
            $description = $details->post_content !== ''?
                sprintf('<div class="%3$s %4$s"><%2$s class="dg_ig_description">%1$s</%2$s></div>',
                    wp_kses_post($details->post_content, 'dggm-gallery-modules'),
                    esc_attr($description_tag),
                    esc_attr($always_description),
                    esc_attr( 'on' !== $show_description ? 'dg-hide-description' : '')
                    ) : '';
        }
        ...
        $data_lightbox_html = $use_lightbox_content === 'on' && $content_box !== '' ?
            'data-sub-html=".dg_ig_content"' : '';

        if ( $load_more_type === false ) {
            $classes = implode(" ", $value);
        }

	    $custom_url = dg_get_custom_url( $use_url, $id );

        $empty_class = $content_box === '' ? ' empty_content' : '';

        $image_url = !empty($media[0]) ? $media[0] : $default_image;

	    $conent_positioning = sprintf( '<%5$s class="dg_ig_content %2$s %3$s %4$s">
                            %1$s
                        </%5$s>',
		    $content_box,
	    ...
	    if('on' === $enable_content_position){
		    $lightbox_caption = 'on' === $use_lightbox_content && $details && '' !== $details->post_excerpt ?
			    sprintf('<div class="ig_is_lightbox"><%2$s class="dg_ig_caption">%1$s</%2$s></div>',
				    wp_kses_post($details->post_excerpt, 'dggm-gallery-modules'),
				    esc_attr($caption_tag)
			    ):"";
		    $lightbox_description = 'on' === $use_lightbox_content && $details && '' !== $details->post_content ?
			    sprintf('<div class="ig_is_lightbox"><%2$s class="dg_ig_description">%1$s</%2$s></div>',
				    wp_kses_post($details->post_content, 'dggm-gallery-modules'),
				    esc_attr($description_tag)
			    ) : '';
		    $content_outside_positioning = sprintf('<figcaption class="dg_ig_content dg_ig_border_for_outside_content">%1$s%2$s</figcaption>', $lightbox_caption, $lightbox_description);
	    }

	    $image = sprintf('<div class="dg_ig_image grid-item %1$s" data-src="%2$s" %3$s>
		    ...
		    'on' === $overlay ? sprintf('<span class="dg-overlay">%1$s</span>','on' === $field_use_icon ? dg_get_processed_font_content($field_font_icon, $content_reveal_icon) : '') : '',
		    esc_attr($image_url),
 ...
 *
 */
function dg_shuffle_assoc($my_array) {
...
/**  Pagination start **/
add_action('wp_ajax_dg_image_gallery_fetch_page_data', 'dg_image_gallery_page_image');
add_action('wp_ajax_nopriv_dg_image_gallery_fetch_page_data', 'dg_image_gallery_page_image');

function dg_image_gallery_page_image() {
...
function dg_get_processed_font_content($field_font_icon, $content_reveal_icon) {
	$icon_data = explode("|",$field_font_icon);
	return sprintf('<div class="dg_ig_icon_wrap %2$s"><span class="et-pb-icon">%1$s</span></div>',
