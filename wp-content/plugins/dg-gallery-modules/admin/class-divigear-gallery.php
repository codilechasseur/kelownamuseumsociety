...
class DiviGear_Media_Category {
	const TAXONOMY = "dg_media_category";
	...
	public function init_media_category() {
		if ( taxonomy_exists( self::TAXONOMY ) ) {
			...
			$labels = [
				'name'              => _x( 'DG Media Categories', 'taxonomy general name', 'dggm-gallery-modules' ),
				'singular_name'     => _x( 'DG Media Categories', 'taxonomy singular name', 'dggm-gallery-modules' ),
				'search_items'      => __( 'Search Category', 'dggm-gallery-modules' ),
				'all_items'         => __( 'All Category', 'dggm-gallery-modules' ),
				'edit_item'         => __( 'Edit Category', 'dggm-gallery-modules' ),
				'update_item'       => __( 'Update Category', 'dggm-gallery-modules' ),
				'add_new_item'      => __( 'Add New Category', 'dggm-gallery-modules' ),
				'new_item_name'     => __( 'New Category Name', 'dggm-gallery-modules' ),
				'menu_name'         => __( 'DG Media Categories', 'dggm-gallery-modules' ),
	...
	public function add_category_filter_in_list_view() {
		...
		if ( 'upload.php' == $pagenow ) {
			...
			$dropdown_options = [
				...
				'name'            => self::TAXONOMY,
				'class'           => 'postform dg-mc-taxonomy-filter',
				'show_option_all' => __( 'View all Category', 'dggm-gallery-modules' ),
				'hide_empty'      => false,
				'hierarchical'    => false,
				'orderby'         => 'name',
				'selected'        => $selected,
	...
	public function enqueue_media_action_for_grid_view_filter() {
		...
		wp_enqueue_style( 'dg-wp-media-category', DGGM_ASSETS_DIR. 'css/dg-wp-media-category.css', [], DGGM_VERSION );
	}
	...
	public function load_media_view_scripts() {
		...
		wp_enqueue_script( 'dg-wp-media-category-media-views', DGGM_ASSETS_DIR.'js/dg-wp-media-category-media-views.js',  [ 'media-views' ], DGGM_VERSION, true );

		wp_localize_script( 'dg-wp-media-category-media-views', 'dg_media_taxonomie_data', [
			self::TAXONOMY => [
				'list_title' => esc_html__( __( 'View all Category', 'dggm-gallery-modules' ), ENT_QUOTES, 'UTF-8' ),
		...
		if ($terms && !is_wp_error($terms)) :
            ...
            <script type="text/javascript">
                jQuery(window).on('load', function () {
                    jQuery('<optgroup id="dg-mc-optgroup-1" label="<?php echo esc_html__('DG Media', 'dggm-gallery-modules'); ?>">').appendTo("select[name='action']");
                    jQuery('<optgroup id="dg-mc-optgroup-2" label="<?php echo esc_html__('DG Media', 'dggm-gallery-modules'); ?>">').appendTo("select[name='action2']");

                    // add categories
					<?php foreach ($terms as $term) : ?>
					<?php
					$optionValue = 'dg-mc-add-' . esc_attr($term->term_taxonomy_id);
					$optionText = esc_js(__('Add', 'dggm-gallery-modules') . ': ' . esc_html($term->name));
					?>
                    jQuery('<option>').val('<?php echo esc_attr($optionValue); ?>').text('<?php echo esc_html($optionText); ?>').appendTo('#dg-mc-optgroup-1');
                    jQuery('<option>').val('<?php echo esc_attr($optionValue); ?>').text('<?php echo esc_html($optionText); ?>').appendTo('#dg-mc-optgroup-2');
					<?php endforeach; ?>
					...
					<?php foreach ($terms as $term) : ?>
					<?php
					$optionValue = 'dg-mc-remove-' . esc_attr($term->term_taxonomy_id);
					$optionText = esc_js(__('Remove', 'dggm-gallery-modules') . ': ' . esc_html($term->name));
					?>
                    jQuery('<option>').val('<?php echo esc_attr($optionValue); ?>').text('<?php echo esc_html($optionText); ?>').appendTo('#dg-mc-optgroup-1');
                    jQuery('<option>').val('<?php echo esc_attr($optionValue); ?>').text('<?php echo esc_html($optionText); ?>').appendTo('#dg-mc-optgroup-2');
					<?php endforeach; ?>
                    ...
                    jQuery('<option>').val('dg-mc-remove-0').text('<?php echo esc_attr(__('Remove all categories', 'dggm-gallery-modules')); ?>').appendTo('#dg-mc-optgroup-1');
                    jQuery('<option>').val('dg-mc-remove-0').text('<?php echo esc_attr(__('Remove all categories', 'dggm-gallery-modules')); ?>').appendTo('#dg-mc-optgroup-2');
                });
            </script>
	...
	public function custom_bulk_action() {
		...
		foreach ( $post_ids as $post_id ) {
			...
			if ( is_numeric( str_replace( 'dg-mc-add-', '', $sAction ) ) ) {
				$nCategory = str_replace( 'dg-mc-add-', '', $sAction );
			...
			} else if ( is_numeric( str_replace( 'dg-mc-remove-', '', $sAction ) ) ) {
				$nCategory = str_replace( 'dg-mc-remove-', '', $sAction );
	...
	public function attachment_fields_to_edit($form_fields, $post) {
		...
		foreach ($terms as $term){
			...
			$_option_data .= '<li id="dg_media_category-' . esc_attr($term->slug) . '" class="popular-category">';
			$_option_data .= '<label class="selectit">';
			$_option_data .= '<input value="' . esc_attr($term->slug) . '" type="checkbox" name="tax_input[dg_media_category][]" id="in-dg_media_category-' . esc_attr($term->slug) . '" ' . (in_array($term->term_taxonomy_id, $selected_cats) ? 'checked="checked"' : '') . '>';
		...
		$html = '<ul class="dg-media-term-list">' . $_option_data . '</ul>';

		$form_fields[self::TAXONOMY] = array(
			'label' => "DG Media Category",
			'input' => 'html',
			'html' => $html,
