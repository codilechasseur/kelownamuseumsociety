...
if( ! class_exists("DG_Setting_Field")) {
    ...
    class DG_Setting_Field {
        ...
        public function dg_setting_field_callback() {
            ?>
                <div class="field-option">
                    <?php DG_Settings::template($this->template_url, $this->data); ?>
                </div>
            <?php
        }
	    ...
	    public function update_license() {
		    ...
		    if ( ! wp_verify_nonce( $data['_wpnonce'], 'dggm_nonce' ) ) {
			    echo wp_json_encode( ['message'=>__( 'invalid nonce','dggm-gallery-modules')] );
		    }
		    $option             = $this->options;
